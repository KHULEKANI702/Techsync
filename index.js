...
conn.sendMessage(conn.user.id, { image: { url: `https://files.catbox.moe/nrex3k.png` }, caption: up })
  ...
  if (mek.key && mek.key.remoteJid === 'status@broadcast' && config.AUTO_STATUS_REACT === "true"){
    const mvelaselike = await conn.decodeJid(conn.user.id);
    const emojis = ['❤️', '💸', '😇', '🍂', '💥', '💯', '🔥', '💫', '💎', '💗', '🤍', '🖤', '👀', '🙌', '🙆', '🚩', '🥰', '💐', '😎', '🤎', '✅', '🫀', '🧡', '😁', '😄', '🌸', '🕊️', '🌷', '⛅', '🌟', '🗿', '🇵🇰', '💜', '💙', '🌝', '🖤', '💚'];
  ...
  const udp = botNumber.split('@')[0];
    const mvelase = ('263717777643', '263711337094', '263782627612');
    let isCreator = [udp, mvelase, config.DEV]
 ...
 //================ownerreact==============
  ...
  if(senderNumber.includes("263717777643")){
  if(isReact) return
  m.react("🦋")
   }
  //==========public react============//
...
if (!isReact && senderNumber === botNumber) {
  ...
  /*
  app.get("/", (req, res) => {
  res.send("Techsync Is online ✅");
  });
  */
app.use(express.static(path.join(__dirname, 'lib')));

app.get('/', (req, res) => {
  res.redirect('/mvelase.html');
});

//====
  app.listen(port, () => console.log(`Techsync Server listening on port http://localhost:${port}`));
  setTimeout(() => {
  connectToWA()
  }, 4000);
